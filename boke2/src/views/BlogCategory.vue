	...
	<el-table :data="tableData" style="width: 100%">
		...
		<el-table-column fixed="right" label="Operations" width="140">
			<template #default="scope">
				...
				<el-button :plain="true" type="text" size="small" @click.prevent="editRow(scope.$index, tableData)">编辑
				</el-button>
			</template>
		</el-table-column>
	</el-table>
...
<script>
	...
	export default {
		...
		created(){
			axios.get('http://127.0.0.1:8088/Blog/findSubPages/1').then(res => {
		...
		methods: {
			...
			page(i){
				axios.get('http://127.0.0.1:8088/Blog/findSubPages/'+i).then(res => {
					this.tableData = res.data.data.content;
				})
			}
